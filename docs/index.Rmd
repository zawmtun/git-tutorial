...
People who do not use Git invariably create some form of ad-hoc version control system. Here is an example by Jenny Bryan[@bryan2018]. Most of us may be familiar with Figure \@ref(fig:git-workflow) (A or B) which shows a hypothetical analysis using a single script file `iris.R`. Following that, the future self or collaborators creates derivative copies of `iris.R` adding file names with initials, dates, or other descriptive characters. It is not hard to imagine that the script files become too complex to manage as the project develops.
...
```{r git-workflow, fig.cap="Version control using file name in (A) solo work or (B) collaborative work; Git workflow in (C) solo work or (D) collaborative work[@bryan2018]", fig.height=2.5, echo=FALSE}
...
```{r eval=FALSE}
rm -rf folder_name # -rf means recursive and force 
rmdir folder_name
```
...
If you find yourself in this situation, you can either initialise a git repo in a different folder or remove the repo in the parent folder. To remove a Git repo, simply locate and delete the associated `.git` folder.
...
You can use the command below to see the Git command quick reference

```
git [command] -help
```

Need more details? Open the online manual page in your default browser using the following commands.
...
5.  Create three files with names of SG hawker dishes: `dish_name1.txt`, `dish_name2.txt`, etc.
...
Use `git log` to get a list of commits with the corresponding details (author, date/time, commit message). This is useful to quickly look at the commit history. `--oneline` generates a succinct output with one row per commit with their commit messages.

```{r eval=FALSE}
git log
git log --oneline
git log --oneline --graph # show the development of commits as a graph
```

If the output is too big to fit in one page, `git log` will show the output the first page and wait for the user to press any key to show the next line or the next page. Press `Q` go back to the command line.
